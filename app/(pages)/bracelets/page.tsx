...
const pageTitle = "Bracelets Collection";
const pageDescription =
  "From delicate chains and classic bangles to bold, modern cuffs.";
...
export default function BraceletsPage() {
  ...
  useEffect(() => {
    dispatch(fetchJewelry({ category: "Bracelets" }));
  }, [dispatch]);

  // DEBUG: Saare product IDs ko data load hone par log karein
  useEffect(() => {
    if (bracelets.length > 0) {
      console.log("🔍 DEBUG: All Products in Bracelets Collection:");
      bracelets.forEach((product, index) => {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        ...
        {bracelets.length === 0 ? (
          <div className="text-center py-20">
            <h2 className="text-2xl font-semibold">No Bracelets Found</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-10">
            {bracelets.map((product) => {
              ...
              if (!isValid) {
                console.warn("⚠️ INVALID PRODUCT ID IN BRACELETS:", {
                  name: product.name,
                  id: product._id,
                });
              }
