...
export default function RingsPage() {
  ...
  // 'items' ko 'rings' naam diya gaya hai
  const {
    items: rings,
    listStatus,
    error,
  ...
  // Sirf "Rings" category fetch ki jaa rahi hai
  useEffect(() => {
  ...
  useEffect(() => {
    if (rings.length > 0) {
      console.log("🔍 DEBUG: All Products in Rings Collection:");
      rings.forEach((product, index) => {
        ...
        {rings.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-10">
            {rings.map((product) => {
              ...
              if (!isValid) {
                console.warn("⚠️ INVALID PRODUCT ID IN RINGS:", {
                  name: product.name,
                  id: product._id,
                });
              }
