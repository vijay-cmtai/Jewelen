...
const pageTitle = "Earrings Collection";
const pageDescription =
  "Studs, hoops, drops and more. Find the perfect pair for every occasion.";
...
export default function EarringsPage() {
  ...
  useEffect(() => {
    dispatch(fetchJewelry({ category: "Earrings" }));
  }, [dispatch]);

  // DEBUG: Saare product IDs ko data load hone par log karein
  useEffect(() => {
    if (earrings.length > 0) {
      console.log("🔍 DEBUG: All Products in Earrings Collection:");
      earrings.forEach((product, index) => {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        ...
        {earrings.length === 0 ? (
          <div className="text-center py-20">
            <h2 className="text-2xl font-semibold">No Earrings Found</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-10">
            {earrings.map((product) => {
              ...
              if (!isValid) {
                console.warn("⚠️ INVALID PRODUCT ID IN EARRINGS:", {
                  name: product.name,
                  id: product._id,
                });
              }
