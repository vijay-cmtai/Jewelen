...
export default function AddInventoryPage() {
  ...
  const { actionStatus, csvHeaders } = useSelector(
  ...
  const handleManualSubmit = (e: FormEvent) => {
    ...
    const jewelryData = {
      ...
      originalPrice: manualForm.originalPrice
        ...
        : undefined,
      tax: manualForm.tax ? parseFloat(manualForm.tax) : 0,
    ...
    dispatch(addJewelry(jewelryData))
      .unwrap()
      .then((response) => {
        toast.success(response.message || "Jewelry added successfully!");
        setManualForm(initialManualState);
      })
      .catch((error) => {
        toast.error(error || "Failed to add jewelry.");
      });
  };
